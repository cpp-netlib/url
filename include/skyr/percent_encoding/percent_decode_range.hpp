...
constexpr inline auto alnum_to_hex(char value) noexcept -> std::expected<std::byte, percent_encode_errc> {
  ...
  return std::unexpected(percent_encoding::percent_encode_errc::non_hex_input);
}
}  // namespace details
...
class percent_decode_iterator {
 public:
  ...
  ///
  using value_type = std::expected<char, percent_encode_errc>;
  ///
  ...
  [[nodiscard]] constexpr auto operator*() const noexcept -> const_reference {
    ...
    if (remainder_[0] == '%') {
      if (remainder_.size() < 3) {
        return std::unexpected(percent_encoding::percent_encode_errc::overflow);
      }
