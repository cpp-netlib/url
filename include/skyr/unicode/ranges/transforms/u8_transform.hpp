...
class u8_transform_iterator {
 public:
  ...
  using value_type = std::expected<char, unicode_errc>;
  /// A reference
  ...
  [[nodiscard]] constexpr auto operator*() const noexcept -> reference {
    constexpr auto u8_code_unit = [](auto code_point, auto octet_index) -> std::expected<char, unicode_errc> {
      if (code_point < 0x80u) {
...
#endif  // SKYR_UNICODE_RANGES_TRANSFORMS_U8_TRANSFORM_HPP
