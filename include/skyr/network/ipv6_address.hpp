...
class ipv6_address {
 ...
 public:
  ...
  [[nodiscard]] auto serialize() const -> std::string {
    ...
    while (true) {
      if (*it == 0) {
        auto index = std::ranges::distance(first, it);
...
constexpr inline auto parse_ipv6_address(std::string_view input, bool* validation_error)
  ...
  if (input.starts_with("::"sv)) {
    ...
    *validation_error |= true;
    return std::unexpected(ipv6_address_errc::does_not_start_with_double_colon);
  }
  ...
  while (it != last) {
    ...
    if ((it != last) && (*it == '.')) {
      if (length == 0) {
        *validation_error |= true;
        return std::unexpected(ipv6_address_errc::empty_ipv4_segment);
      }
...
#endif  // SKYR_NETWORK_IPV6_ADDRESS_HPP
